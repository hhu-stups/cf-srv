...
class DroneEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        super().reset()
        self.base_x = random.randint(0, WIDTH-1)
        self.base_y = random.randint(0, DEPTH-1)
        self.x = self.base_x
        self.y = self.base_y
    ...
    # 0: Forward, 1: Backward, 2: Left, 3: Right, 4: Land
    def step(self, action):
        ...
        if len(self.explored) == DEPTH * WIDTH:
...
def main():
    if len(sys.argv) >= 2 and sys.argv[1] == 'train':
        ...
        model.learn(int(5_000_000), callback=checkpoint_callback, tb_log_name="new_dqn", progress_bar=True)
        ...
        for i in range(1):
            ...
            for step in range(100):
                ...
                forward, right, backward, left = env._get_surrounding_fields()
                print("------------------")
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
                ...
                while True:
                    ...
                    response = json.dumps({
                        'op': '$setup_constants',
                        'delta': 0,
                        'predicate': "BASE_POSITION = {0} |-> {1}".format(env.base_x+1, env.base_y+1),
                        'done': 'false'
                    }) + "\n"
                    ...
                    request = json.loads(read_line(client_socket))
                    response = json.dumps({
                        'op': 'SYNCHRONIZE_BATTERY',
                        'delta': delta,
                        'predicate': "pbattery = {0}".format(env.battery),
                        'done': 'false'
                    }) + "\n"
                    client_socket.sendall(response.encode('utf-8'))
                    ...
                    while not done and not finished:
                        ...
                        if 'SYNCHRONIZE_BATTERY' in operations_list:
                            response = json.dumps({
                                'op': 'SYNCHRONIZE_BATTERY',
                                'delta': delta,
                                'predicate': "pbattery = {0}".format(env.battery),
                        ...
                        if 'UPDATE_POSITION' in operations_list:
                            response = json.dumps({
                                'op': 'UPDATE_POSITION',
                                'delta': 0,
                                'predicate': "x = {0} & y = {1}".format(env.x+1, env.y+1),
                                'done': 'false'
                            }) + "\n"
                            client_socket.sendall(response.encode('utf-8'))
                            request = json.loads(read_line(client_socket))
                        ...
                        if 'SYNCHRONIZE_BATTERY' in operations_list:
                            ...
                            enabled_operations = request['enabledOperations']
                            operations_list = enabled_operations.split(",")
                        if 'OBSERVE' in operations_list:
                            forward, right, backward, left = env._get_surrounding_fields()
                            response = json.dumps({
                                'op': 'OBSERVE',
                                'delta': 0,
                                'predicate': "left_sensor = {0} & right_sensor = {1} & forward_sensor = {2} & backward_sensor = {3}".format(left, right, forward, backward),
                                'done': 'false'
                            }) + "\n"
                            client_socket.sendall(response.encode('utf-8'))
                            request = json.loads(read_line(client_socket))
